...
function readExportRangeFromDom() {
    ...
    if (!desdeStr || !hastaStr) {
        return {
            ok: false,
            error: "Indica fecha «Desde» y «Hasta» para exportar."
        };
    }
    ...
    if (!desdeParsed || !hastaParsed) {
        return {
            ok: false,
            error: "Las fechas de exportación no son válidas."
        };
    }
    ...
    if (spanDays > 365 * 5) {
        return {
            ok: false,
            error: "El rango máximo para exportar es 5 años."
        };
    }
    return {
        ok: true,
...
function readPlanesRangeFromDom() {
    ...
    if (periodo !== "custom") {
        ...
        if (!Number.isFinite(meses) || meses <= 0) {
            return {
                ok: false,
                error: "Período de planes no válido."
            };
        }
        const desde = new Date(hoy.getFullYear(), hoy.getMonth() - meses + 1, 1);
        return {
            ok: true,
            desde0: startOfDay(desde),
            hastaEnd: endOfDay(hoy)
        };
    }
    ...
    if (!desdeStr || !hastaStr) {
        return {
            ok: false,
            error: "Indica fecha «Desde» y «Hasta» en el filtro de planes."
        };
    }
    const desdeParsed = parseFechaInput(desdeStr);
    const hastaParsed = parseFechaInput(hastaStr);
    if (!desdeParsed || !hastaParsed) {
        return {
            ok: false,
            error: "Las fechas del filtro de planes no son válidas."
        };
    }
    const desde0 = startOfDay(desdeParsed);
    const hastaEnd = endOfDay(hastaParsed);
    if (desde0 > hastaEnd) {
        return {
            ok: false,
            error: "En planes: «Desde» no puede ser posterior a «Hasta»."
        };
    }
    return {
        ok: true,
...
function readChartRangeFromDom({
    ...
    if (!periodo) {
        return {
            ok: true,
            desde0: fallbackDesde0,
            hastaEnd: fallbackHastaEnd
        };
    }
    ...
    if (periodo !== "custom") {
        ...
        if (!Number.isFinite(meses) || meses <= 0) {
            return {
                ok: false,
                error: "Período no válido para exportar."
            };
        }
        const desde = new Date(hoy.getFullYear(), hoy.getMonth() - meses + 1, 1);
        return {
    ...
    if (!desdeStr || !hastaStr) {
        return {
            ok: false,
            error: "Indica fecha «Desde» y «Hasta» del gráfico para exportar."
        };
    }
    const desdeParsed = parseFechaInput(desdeStr);
    const hastaParsed = parseFechaInput(hastaStr);
    if (!desdeParsed || !hastaParsed) {
        return {
            ok: false,
            error: "Las fechas del gráfico no son válidas."
        };
    }
    const desde0 = startOfDay(desdeParsed);
    const hastaEnd = endOfDay(hastaParsed);
...
function filasResumenActivosVencidos(usuarios, fechaCorteInicioDia) {
    ...
    return [{
            Métrica: "Fecha de corte (hasta)",
            Valor: fechaCorteInicioDia.toLocaleDateString("es-CO")
        },
        {
            Métrica: "Socios activos (al corte)",
            Valor: activos
        },
        {
            Métrica: "Socios vencidos (al corte)",
            Valor: vencidos
        },
        {
            Métrica: "Sin fecha fin de membresía",
            Valor: sinFecha
        },
        {
            Métrica: "Total socios en base",
...
function appendSheet(wb, rows, name) {
    ...
    const ws = XLSX.utils.json_to_sheet(rows.length ? rows : [{
        Mensaje: "Sin datos en este rango"
    }]);
...
function exportActivosVencidosExcel({
    usuarios,
    pagos,
    planes,
    desde0,
...
function exportMetodosPagoExcel({
    usuarios,
    pagos,
    planes,
    desde0,
    ...
    const detalle = filasPagosDetalle(pagosF, uById, pById, {
        soloCamposPublicos: true
    });
...
function exportPlanesVendidosExcel({
    usuarios,
    pagos,
    planes,
    desde0,
...
function exportTodoExcel({
    ...
    const portada = [{
            Campo: "Rango exportación (desde)",
            Valor: desdeStr
        },
        {
            Campo: "Rango exportación (hasta)",
            Valor: hastaStr
        },
        {
            Campo: "Nota",
            Valor: "Pagos filtrados por fecha de pago. Socios: con actividad o fechas en el rango."
        },
    ];
    const wb = XLSX.utils.book_new();
