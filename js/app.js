...
} from "./importar-excel.js";
import {
    inicializarMigracionRadical
} from "./migrar-excel-radical.js";
import {
    inicializarExportLegacyFirestore
} from "./export-legacy-firestore.js";
import {
...
/** Periodo para KPI de ingresos y ticket (fecha de pago) */
let filtroKpiIngresos = "mes_actual";
...
function showSection(sectionId) {
    ...
    // ApexCharts calcula mal tamaño/series si el contenedor estaba en display:none (p. ej. carga en Inicio).
    if (sectionId === "dashboard") {
        requestAnimationFrame(() => {
            requestAnimationFrame(() => {
                const u = window.__usuarios || [];
                const p = window.__pagos || [];
                const pl = window.__planes || [];
                if (!u.length && !p.length && !pl.length) return;
                renderDashboardCharts(u, p, pl);
            });
        });
    }
}
        ...
        list.innerHTML = visibles
            .map(
                (u) => {
                    ...
                    const msgWa = mensajeWhatsAppPara("usuarios", u);
...
const WA_MENSAJES_STORAGE_KEY = "radical_wa_mensajes_v2";

const WA_MENSAJES_DEFAULT = {
    usuarios: "te contacto desde Radical Training.",
    vencidos: "tu membresía en Radical Training venció. ¡Te esperamos para seguir entrenando!",
    proximos: "tu membresía en Radical Training vence. ¡Te esperamos para seguir entrenando!",
};

/** Quita placeholders viejos y un "Hola" inicial; el sistema los agrega solo. */
function limpiarCuerpoMensajeWa(texto) {
    return String(texto || "")
        .replaceAll("{nombre}", "")
        .replaceAll("{fecha}", "")
        .replace(/^hola\b[\s,]*/i, "")
...
function obtenerMensajesWhatsApp() {
    try {
        const raw = localStorage.getItem(WA_MENSAJES_STORAGE_KEY);
        if (!raw) return { ...WA_MENSAJES_DEFAULT };
        const parsed = JSON.parse(raw);
        return {
            usuarios: limpiarCuerpoMensajeWa(parsed.usuarios) || WA_MENSAJES_DEFAULT.usuarios,
            vencidos: limpiarCuerpoMensajeWa(parsed.vencidos) || WA_MENSAJES_DEFAULT.vencidos,
            proximos: limpiarCuerpoMensajeWa(parsed.proximos) || WA_MENSAJES_DEFAULT.proximos,
        };
    } catch {
        return { ...WA_MENSAJES_DEFAULT };
    }
}

function guardarMensajesWhatsApp(mensajes) {
    localStorage.setItem(
        WA_MENSAJES_STORAGE_KEY,
        JSON.stringify({
            usuarios: limpiarCuerpoMensajeWa(mensajes.usuarios),
            vencidos: limpiarCuerpoMensajeWa(mensajes.vencidos),
            proximos: limpiarCuerpoMensajeWa(mensajes.proximos),
        })
    );
}

function mensajeWhatsAppPara(tipo, usuario, fechaStr = "") {
    const plantillas = obtenerMensajesWhatsApp();
    const cuerpo = limpiarCuerpoMensajeWa(plantillas[tipo] || WA_MENSAJES_DEFAULT[tipo] || "");
    const nombre = (usuario?.nombre || "").trim() || "cliente";

    let msg = `Hola ${nombre}`;
    if (cuerpo) msg += `, ${cuerpo}`;

    const necesitaFecha = tipo === "vencidos" || tipo === "proximos";
    if (necesitaFecha && fechaStr) {
        msg = msg.replace(/[.\s]+$/u, "");
        msg += `. Fecha: ${fechaStr}.`;
    }
...
function rellenarFormularioMensajesWhatsApp(mensajes = obtenerMensajesWhatsApp()) {
    const elUsuarios = document.getElementById("waMsgUsuarios");
    const elVencidos = document.getElementById("waMsgVencidos");
    const elProximos = document.getElementById("waMsgProximos");
    if (elUsuarios) elUsuarios.value = mensajes.usuarios;
    if (elVencidos) elVencidos.value = mensajes.vencidos;
    if (elProximos) elProximos.value = mensajes.proximos;
}

function inicializarMensajesWhatsApp() {
    rellenarFormularioMensajesWhatsApp();

    document.getElementById("formMensajesWhatsApp")?.addEventListener("submit", (e) => {
        e.preventDefault();
        const mensajes = {
            usuarios: document.getElementById("waMsgUsuarios")?.value.trim() || "",
            vencidos: document.getElementById("waMsgVencidos")?.value.trim() || "",
            proximos: document.getElementById("waMsgProximos")?.value.trim() || "",
        };
        if (!limpiarCuerpoMensajeWa(mensajes.usuarios) ||
            !limpiarCuerpoMensajeWa(mensajes.vencidos) ||
            !limpiarCuerpoMensajeWa(mensajes.proximos)) {
            mostrarMensaje("mensajeWaPlantillas", "✗ Completa los tres mensajes antes de guardar.", "error");
            return;
        }
        guardarMensajesWhatsApp(mensajes);
        rellenarFormularioMensajesWhatsApp(obtenerMensajesWhatsApp());
        mostrarMensaje("mensajeWaPlantillas", "✓ Mensajes guardados en este navegador.", "exito");
        if (window.__usuarios) {
            const filtroUsuarios = document.getElementById("buscarUsuario")?.value || "";
            renderUsuarios(window.__usuarios, filtroUsuarios);
            actualizarDashboard(window.__usuarios);
        }
    });

    document.getElementById("btnRestablecerMensajesWa")?.addEventListener("click", () => {
        guardarMensajesWhatsApp({ ...WA_MENSAJES_DEFAULT });
        rellenarFormularioMensajesWhatsApp(WA_MENSAJES_DEFAULT);
        mostrarMensaje("mensajeWaPlantillas", "✓ Se restablecieron los mensajes predeterminados.", "exito");
        if (window.__usuarios) {
            const filtroUsuarios = document.getElementById("buscarUsuario")?.value || "";
            renderUsuarios(window.__usuarios, filtroUsuarios);
            actualizarDashboard(window.__usuarios);
...
function renderVencidosPaginados(vencidosList) {
    ...
    tbodyVencidos.innerHTML = visibles
        .map((u) => {
            ...
            const msg = mensajeWhatsAppPara("vencidos", u, finStr);
...
function renderProximosPaginados(proximosVencer) {
    ...
    tbodyProximos.innerHTML = visibles
        .map((u) => {
            const finFecha = u.fechaFinMembresia ?.toDate ?.();
            const finStr = finFecha ? finFecha.toLocaleDateString("es") : "";
            const msg = mensajeWhatsAppPara("proximos", u, finStr);
            const wa = urlWhatsApp(u.telefono, msg);
            const linkWa = wa ?
                `<a href="${wa}" target="_blank" rel="noopener" class="link-whatsapp" title="Abrir WhatsApp"><i class="fa-brands fa-whatsapp" aria-hidden="true"></i><span class="visually-hidden">WhatsApp</span></a>` :
                "—";
...
function construirBucketsIngresosRango(desdeStr, hastaStr) {
    ...
    if (!desde || !hasta) {
        return {
            error: "Indica fecha «Desde» y «Hasta».",
            buckets: [],
            desde0: null,
            hastaEnd: null,
            agrupacion: null
    ...
    if (spanDays > 365 * 5) {
        return {
            error: "El rango máximo es 5 años. Reduce el intervalo.",
            buckets: [],
            desde0,
            hastaEnd,
            agrupacion: null
    ...
    if (spanDays <= 62) {
        for (let d = new Date(desde0); d <= hastaEnd; d.setDate(d.getDate() + 1)) {
            ...
            const label = inicio.toLocaleDateString("es-CO", {
                day: "numeric",
                month: "short"
            });
            buckets.push({
        ...
        while (cur <= hastaEnd) {
            ...
            if (inicio <= fin) {
                ...
                const label = cur.toLocaleDateString("es-CO", {
                    month: "long",
                    year: "numeric"
                });
                buckets.push({
...
/** Filtra pagos por fecha de pago para los KPI de ingresos / ticket */
function rangoParaKpiIngresos(filtro, hoy, desdeStr, hastaStr) {
    ...
    if (filtro === "custom") {
        ...
        if (custom.error) {
            return {
                ...
                desde0: null,
                hastaEnd: null,
                hint: "Rango personalizado"
            };
        }
        const d0 = parseFechaInput(desdeStr);
        const d1 = parseFechaInput(hastaStr);
        const hint = d0 && d1 ?
            `${d0.toLocaleDateString("es-CO")} — ${d1.toLocaleDateString("es-CO")}` :
            "Rango personalizado";
        return {
            error: null,
    ...
    if (filtro === "mes_actual") {
        const desde0 = startOfDay(new Date(h.getFullYear(), h.getMonth(), 1));
        const ultimoDia = new Date(h.getFullYear(), h.getMonth() + 1, 0);
        return {
            error: null,
            desde0,
            hastaEnd: endOfDay(ultimoDia),
            hint: "Mes calendario actual",
        };
    }
    if (filtro === "mes_hasta_hoy") {
        const desde0 = startOfDay(new Date(h.getFullYear(), h.getMonth(), 1));
        return {
            error: null,
            desde0,
            hastaEnd: endOfDay(h),
            hint: "Mes actual hasta hoy"
        };
    }
    if (filtro === "mes_anterior") {
        const desde0 = startOfDay(new Date(h.getFullYear(), h.getMonth() - 1, 1));
        const ultimoDia = new Date(h.getFullYear(), h.getMonth(), 0);
        return {
            error: null,
            desde0,
            hastaEnd: endOfDay(ultimoDia),
    ...
    return rangoParaKpiIngresos("mes_actual", hoy, desdeStr, hastaStr);
}
...
function renderDashboardCharts(usuarios, pagos, planes) {
    ...
    // Corte del periodo (día «Hasta»): activos/vencidos respecto a esa fecha, toda la base (como Inicio; sin filtrar por pagos)
    const corteDonut = rangoEstado.hastaEnd ? startOfDay(new Date(rangoEstado.hastaEnd)) : hoy;
    const activosChart = usuarios.filter((u) => {
        const fin = toDateSafe(u.fechaFinMembresia);
        return fin && fin >= corteDonut;
    }).length;
    const vencidosChart = usuarios.filter((u) => {
        const fin = toDateSafe(u.fechaFinMembresia);
        return fin && fin < corteDonut;
    }).length;

    // KPIs superiores: mismo criterio que Inicio (toda la base, membresía vs hoy)
    const hoyKpi = new Date();
    hoyKpi.setHours(0, 0, 0, 0);
    const totalActivosKpi = usuarios.filter((u) => {
        const fin = toDateSafe(u.fechaFinMembresia);
        return fin && fin >= hoyKpi;
    }).length;
    const totalVencidosKpi = usuarios.filter((u) => {
        const fin = toDateSafe(u.fechaFinMembresia);
        return fin && fin < hoyKpi;
    }).length;
    ...
    const errKpiIng = document.getElementById("kpiIngresosRangoError");
    const rangoKpi = rangoParaKpiIngresos(
        filtroKpiIngresos,
        hoy,
        document.getElementById("kpiFechaDesde") ?.value || "",
        document.getElementById("kpiFechaHasta") ?.value || ""
    );
    if (errKpiIng) {
        if (rangoKpi.error) {
            errKpiIng.textContent = rangoKpi.error;
            errKpiIng.hidden = false;
        } else {
            errKpiIng.textContent = "";
            errKpiIng.hidden = true;
        }
    }
    const pagosKpi = !rangoKpi.error && rangoKpi.desde0 && rangoKpi.hastaEnd ?
        pagos.filter((p) => {
            const fecha = toDateSafe(p.fechaPago);
            return fecha && fecha >= rangoKpi.desde0 && fecha <= rangoKpi.hastaEnd;
        }) :
        [];
    const ingresoKpi = pagosKpi.reduce((acc, p) => acc + Number(p.montoPagado || 0), 0);
    const ticketKpi = pagosKpi.length ? ingresoKpi / pagosKpi.length : 0;
    ...
    const kpiIngresoHint = document.getElementById("kpiIngresoPeriodoHint");
    const kpiTicketHint = document.getElementById("kpiTicketPeriodoHint");
    if (kpiActivos) kpiActivos.textContent = totalActivosKpi;
    if (kpiVencidos) kpiVencidos.textContent = totalVencidosKpi;
    if (kpiIngresoMes) kpiIngresoMes.textContent = formatearCOP(rangoKpi.error ? 0 : ingresoKpi);
    if (kpiTicketPromedio) kpiTicketPromedio.textContent = formatearCOP(rangoKpi.error ? 0 : ticketKpi);
    const hintTxt = rangoKpi.error ? "Indica fechas válidas" : rangoKpi.hint;
    if (kpiIngresoHint) kpiIngresoHint.textContent = hintTxt;
    if (kpiTicketHint) kpiTicketHint.textContent = hintTxt;
    ...
    chartEstadoMembresias = new ApexCharts(document.querySelector("#chartEstadoMembresias"), {
        ...
        series: [activosChart, vencidosChart],
    ...
    chartIngresosMensuales = new ApexCharts(document.querySelector("#chartIngresosMensuales"), {
        ...
        xaxis: {
            ...
            labels: {
                ...
                style: {
                    colors: "#9a958a",
                    fontSize: esMobileGrafico ? "10px" : "12px"
                },
            },
        },
        yaxis: {
    ...
    chartPlanesVendidos = new ApexCharts(document.querySelector("#chartPlanesVendidos"), {
        ...
        series: [{
            name: "Pagos",
            data: topPlanes.length ? topPlanes.map((p) => p[1]) : [0]
        }],
        xaxis: {
...
function actualizarPanelKpiIngresosCustom() {
    const select = document.getElementById("filtroKpiIngresos");
    const panel = document.getElementById("kpiIngresosRangoCustom");
    if (!select || !panel) return;
    const esCustom = select.value === "custom";
    panel.hidden = !esCustom;
    if (!esCustom) return;
    const desdeIn = document.getElementById("kpiFechaDesde");
    const hastaIn = document.getElementById("kpiFechaHasta");
    if (desdeIn && hastaIn && (!desdeIn.value || !hastaIn.value)) {
        const h = new Date();
        h.setHours(0, 0, 0, 0);
        hastaIn.value = aIsoFechaLocal(h);
...
function inicializarFiltroKpiIngresos() {
    const sel = document.getElementById("filtroKpiIngresos");
    if (!sel) return;
    sel.value = filtroKpiIngresos;
    actualizarPanelKpiIngresosCustom();
    sel.addEventListener("change", () => {
        filtroKpiIngresos = sel.value;
        actualizarPanelKpiIngresosCustom();
        renderDashboardCharts(window.__usuarios || [], window.__pagos || [], window.__planes || []);
    });
    document.getElementById("btnAplicarRangoKpiIngresos") ?.addEventListener("click", () => {
        if (filtroKpiIngresos === "custom") {
            renderDashboardCharts(window.__usuarios || [], window.__pagos || [], window.__planes || []);
        }
    });
}
...
inicializarFiltroKpiIngresos();
inicializarMensajesWhatsApp();
inicializarDashboardExport(() => ({
