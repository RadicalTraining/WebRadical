...
function rangoParaKpiIngresos(filtro, hoy, desdeStr, hastaStr) {
  ...
  if (filtro === "custom") {
    ...
    if (custom.error) {
      return { error: custom.error, desde0: null, hastaEnd: null, hint: "Rango personalizado" };
    }
    ...
    const hint = d0 && d1
      ...
      : "Rango personalizado";
    return { error: null, desde0: custom.desde0, hastaEnd: custom.hastaEnd, hint };
  }
  ...
  if (filtro === "mes_hasta_hoy") {
    const desde0 = startOfDay(new Date(h.getFullYear(), h.getMonth(), 1));
    return { error: null, desde0, hastaEnd: endOfDay(h), hint: "Mes actual hasta hoy" };
  }
  ...
  if (filtro === "30d") {
    ...
    return { error: null, desde0: startOfDay(d), hastaEnd: endOfDay(h), hint: "Últimos 30 días" };
  }
  if (filtro === "90d") {
    const d = new Date(h);
    d.setDate(d.getDate() - 89);
    return { error: null, desde0: startOfDay(d), hastaEnd: endOfDay(h), hint: "Últimos 90 días" };
  }
